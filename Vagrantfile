...
vms = {
  'pgpool01' => { 'memory' => "#{pgpool_memory}", 'disk' => "#{pgpool_disk_size}", 'ip' => '192.168.1.181', 'cpu' => "#{pgpool_cpus}", 'cpu_cap' => "#{pgpool_cpu_cap}" },
  'pgpool02' => { 'memory' => "#{pgpool_memory}", 'disk' => "#{pgpool_disk_size}", 'ip' => '192.168.1.182', 'cpu' => "#{pgpool_cpus}", 'cpu_cap' => "#{pgpool_cpu_cap}" },
  'pgpool03' => { 'memory' => "#{pgpool_memory}", 'disk' => "#{pgpool_disk_size}", 'ip' => '192.168.1.183', 'cpu' => "#{pgpool_cpus}", 'cpu_cap' => "#{pgpool_cpu_cap}" },
  }

# pgpool_vip = '192.168.1.190'
...
Vagrant.configure("2") do |config|
  ...
  vms.each do |name, conf|
    config.vm.define "#{name}" do |k|
      ...
      # k.vm.network 'public_network', ip: "#{pgpool_vip}", bridge: "eno1"
